...
with open("%s/juniorschool.bat"%PREFIX, "w") as down:
    ...
    if r.status_code == 200:
        for json_str in extract_all('=', ";", r.content):
            ...
            for area in json_obj:
                [num, name] = area.split(':')
                res = requests.get('http://support.renren.com/juniorschool/%s.html'%num)
                if res.status_code == 200:
                    print num, name
                    down.write('wget http://support.renren.com/juniorschool/%s.html -O %s/juniorschool/%s%s.html\n'%(num, PREFIX, name, num))
